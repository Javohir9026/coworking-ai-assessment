...
export const useReservationsStore = defineStore('reservations', () => {
  ...
  async function fetchMine(page = meta.value.page, pageSize = meta.value.pageSize): Promise<void> { isLoading.value = true; errorMessage.value = null; try { const result = await useApiClient().request<PaginatedResult<Reservation>>('/reservations', { query: { page, pageSize } }); items.value = result.items; meta.value = result.meta } catch (error: unknown) { items.value = []; errorMessage.value = error instanceof Error ? error.message : 'Reservations could not be loaded.' } finally { isLoading.value = false } }
  ...
  return { items, isLoading, errorMessage, successMessage, meta, fetchMine, fetchOne, createHold, create, simulatePayment }
})
