...
export const useResourceStore = defineStore('resource', () => {
  ...
  async function fetchResources(page = meta.value.page, pageSize = meta.value.pageSize): Promise<void> {
    isLoading.value = true
    errorMessage.value = null
    try {
      const api = useApiClient()
      const result = await api.request<PaginatedResult<Resource>>('/resources', { query: { page, pageSize } })
