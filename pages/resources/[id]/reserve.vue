...
async function create(payload: { resourceId: string; startAt: string; endAt: string }): Promise<void> { if (!resource.value) return; const hours = Math.ceil((new Date(payload.endAt).getTime() - new Date(payload.startAt).getTime()) / 3600000); const reservation = await reservations.create({ ...payload, totalPriceMinor: hours * resource.value.hourlyPriceMinor, currency: resource.value.currency }); createdId.value = reservation.id }
</script>
