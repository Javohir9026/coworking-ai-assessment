...
const isEmpty = computed(
  () => !submitted.value && email.value.length === 0 && password.value.length === 0
)
  ...
  <main class="mx-auto flex min-h-screen max-w-md items-center px-6 py-12">
    <section class="w-full rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
      ...
      <p class="mt-2 text-sm text-slate-600">
        Use a seeded assessment account or enter credentials.
      </p>
      ...
      <form class="mt-5 space-y-4" @submit.prevent="submit">
        ...
        <button
          type="submit"
          class="w-full rounded-lg bg-indigo-600 px-4 py-2.5 font-semibold text-white disabled:cursor-not-allowed disabled:bg-indigo-300"
