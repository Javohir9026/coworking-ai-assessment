...
const estimatedTotalCents = computed(() => {
  ...
  const durationMs = new Date(endLocal.value).getTime() - new Date(startLocal.value).getTime()
  return durationMs > 0 ? Math.floor((props.resource.hourlyPriceMinor * durationMs + 1_800_000) / 3_600_000) : 0
})
...
<template>
  <aside class="fixed inset-y-0 right-0 z-30 w-full max-w-md overflow-y-auto border-l border-slate-200 bg-white p-6 shadow-2xl" aria-label="Create reservation">
    ...
    <div v-else class="mt-6 rounded-xl bg-indigo-50 p-4"><p class="font-semibold text-indigo-950">{{ resource?.name }}</p><p class="mt-1 text-sm text-indigo-800">A 10-minute temporary hold is created when you submit this form.</p></div>
